				...
				<div class="postLayoutView" style="padding: 0%; border: solid black 2px ">
					...
					<div class="post MainWrap mb-3" style="margin-top: 10px; margin-left: 10px; margin-bottom: 20px">
						<div class="profileImgArea" id="profileImgArea" style="display: inline-block; width: 40px; height: 40px; border-radius: 70%; overflow: hidden;">
							 ...
							 <img src="${contextPath}/resources/profileImage/${notice.memberProfile}" 
									data-toggle="modal" data-target="#writerModal" id="writerImg" class="writerImg"
									style="width: 100%; height: 100%; object-fit: cover; position: relative;" name="${notice.memberNo}"> 
						...
						<div class="profileNameArea  nanum" id="profileNameArea" style="display: inline-block; width: 70%; height: 100%; position: relative;">
							...
							<div style="width: 60% !important;">
								<p style="width:100%">
									<fmt:formatDate value="${notice.boardWriteDt}" pattern="yyyy년 MM월 dd일 aa hh:mm" />
								</p>
							</div>
						</div>
						...
						<c:if test="${loginMember.memberNickname ne notice.boardWriter}">
							<c:if test="${memGradeInSt.citizenGrade eq 'M'}">
								...
								<c:if test="${notice.boardLevel eq '1'}">
								<div style="display: inline-block; width: 5%; margin-bottom: 0px; height: 50px; float: right;">
									<div>
										<div class="optionChevron">
											...
											<div id="postMenu" class="hide nanum form-control" style="width: 130px; height: auto; 
												...
												<ul>
													<li><a class="PinPost" name="${notice.boardNo}" style="color: #5a5a5a;" class="lihover">공지사항 해제</a></li>
								 ...
								 <c:if test="${notice.boardLevel eq '0'}">
								<div style="display: inline-block; width: 5%; margin-bottom: 0px; height: 50px; float: right;">
									<div>
										<div class="optionChevron">
											<img src="${contextPath}/resources/img/download.png"
											...
											<div id="postMenu" class="hide nanum form-control" style="width: 130px; height: auto; 
													 float: right; right: 25px; bottom: 12px; font-size: 14px; margin-right: 15px; position: relative">
												<ul>
													<li><a class="PinPost" name="${notice.boardNo}" style="color: #5a5a5a;" class="lihover">공지사항 등록</a></li>
													<li><a class="deletePost" name="${notice.boardNo}" style="color: #5a5a5a;" class="lihover">글 삭제</a></li>
						...
						<c:if test="${loginMember.memberNickname eq notice.boardWriter}">
						  <!--  로그인회원이 골목 대장 일때 -->
						  <c:if test="${memGradeInSt.citizenGrade eq 'M'}">
					...
					<div class="postMainWrap nanum" style="width: 100%">
						<div style="padding-left: 10px; padding-right: 10px; width: 100%">
							...
							<c:choose>
								<c:when test="${notice.typeNo eq '6'}">  <!-- 지도 출력 양식 -->	 
									 	...
									 	<div class="mapShowArea" name="${notice.sketchUrl}" style="height: 100px; width:100%; margin-bottom: 10px;" data-toggle="modal" data-target="#mapShowModal">
											...
											<div class="profileNameArea nanum" id="profileNameArea" style="display: inline-block; width: 81%; margin-bottom: 0px; height: 100%;
												...
												<div style="margin-left: 30px;">
													<p class="mapShowAddress" name="${notice.mapAddress}" style="margin-top: 20px; font-size:17px;  color:black;" data-toggle="modal" data-target="#mapShowModal">${notice.mapAddress}</p>
												</div>
											</div>	
										</div>
								</c:when>
									...
									<c:forEach items="${dutch}" var="dut">
										<c:if test="${notice.boardNo == dut.boardNo }">
											...
											<table class="table table-hover">
												<thead>
													<tr class="table-primary">
														<th colspan="5">${dut.dutchTotal}원 / ${dut.dutchCount}명</th>
													</tr>
												</thead>
												<tbody>
								...
								<c:when test="${notice.typeNo eq '5'}">  <!-- 스케치 출력 양식 -->	
								
									${notice.boardContent}  
									<img src="${contextPath}/resources/sketchImage/${notice.sketchUrl}">
									
								</c:when>
								<c:when test="${notice.typeNo eq '3'}">  <!-- 투표 출력 양식 -->	
								
								 	 ${notice.boardContent}  <!-- 투표 게시글 내용 -->	
								 	 
								 <c:set var="doneLoop" value="false"/>	   <!-- 반복 제어를 위한 변수 선언 -->	
							 	  <c:forEach var="vote" items="${vote}" varStatus="vs">
							 	 	 <c:if test="${not doneLoop}">			  
							 		  <c:if test="${vote.boardNo eq notice.boardNo}">
										...
										<div class="voteMainWrap nanum" style="border: 1px solid #ced4da;  height: auto; width:100%; margin-top:20px;">
											...
											<div style=" height: auto; width:100%; border: 1px solid #ced4da; border-bottom: 0; border-left: 0; border-right: 0;">
											 ...
											 <c:forEach var="voteOption" items="${voteOption}" varStatus="vs">
							 				 	<c:if test="${voteOption.boardNo eq notice.boardNo}">
												<!-- 선택지 1 -->
												<div style="margin-top: 10px;  margin-bottom: 30px;">
													...
													<div style="width:80%; margin-left: 60px;" >
														...
														<c:if test="${vote.anonymity eq 'N'}">
														 	...
														 	<p style="font-size:13px;" name="${vote.anonymity}">
														 	<c:forEach var="voteMemList" items="${voteMemList}" varStatus="vs">
														 		<c:if test="${voteMemList.voteNo eq voteOption.voteNo}">
														 		  ${voteMemList.memberNickname},  
														 	    </c:if>
														 	</c:forEach>
														 	</p>
														 ...
														 <c:if test="${vote.anonymity eq 'Y'}">
														 	<p style="font-size:13px;" name="${voteOption.anonymity}"></p>
														 </c:if>	
													</div>
												</div>
												<!-- 선택지 1 -->
					...
					<div class="postCountView mt-3"
						...
						<div class="commentArea nanum"
							...
							<c:choose>
								<c:when test="${notice.replyCount eq '0'}">
									<!-- 댓글 개수 0 일때 공백처리 -->
									<p class="commentCount mb-3" style="margin-bottom: 20px;">댓글</p>
								</c:when>
								<c:otherwise>
									<p class="commentCount mb-3" style="margin-bottom: 0;">댓글${notice.replyCount}</p>
								</c:otherwise>
							</c:choose>
						</div>
						...
						<c:if test="${memGradeInSt.citizenStatus eq 'Y'}">
							...
							<button type="submit" class="btn nanum"
								...
								<c:choose>
									<c:when test="${notice.thumbStatus eq 'Y'}">
										<!-- 내가 좋아요 누른 게시글 활성화로 표시 -->
										<img class="likeBtn shake" name="${notice.boardNo}" 
							...
							<c:choose>
								<c:when test="${notice.thumbCount eq '0'}">
									<!-- 좋아요 개수 0 일때 공백처리 -->
									<p class="likeCount"
								...
								<c:otherwise>
									<p class="likeCount"
										style="margin-bottom: 0; display: inline-block;">${notice.thumbCount}</p>
								</c:otherwise>
							</c:choose>
							</div>
						</c:if>
					...
					<div class="CommentWrap" style="display: none;">
						...
						<c:if test="${memGradeInSt.citizenStatus eq 'Y'}">
							...
							<div class="inputCommentWrap" style="border: 1px solid #ced4da;">
								...
								<div style="width: 20%; display: inline-block; position: absolute;">
									<button type="submit" class="btn btn-secondary nanum commentBtn" id="commentBtn" name="${notice.boardNo}"
						...
						<c:if test="${!empty reply}">
							<c:forEach var="reply" items="${reply}" varStatus="vs">
								<c:if test="${reply.boardNo eq notice.boardNo}">
									<c:if test="${reply.replyLevel eq '1'}">
										<!-- 댓글보기 -->
																	...
																	<c:choose>
																		<c:when test="${reply.thumbStatus eq 'Y'}">
																			...
																			<img class="likeBtn2 shake"
																				src="${contextPath}/resources/img/like2.png" name="${reply.replyNo}" id="${notice.boardNo}"
																				style="bottom: 3px; right:10px;">
																		</c:when>
																		<c:otherwise>
																			<img class="likeBtn2 shake"
																				src="${contextPath}/resources/img/like.png" name="${reply.replyNo}" id="${notice.boardNo}"
																				style="bottom: 3px; right:10px;">
																		</c:otherwise>
																	</c:choose>
																</div>
											...
											<div class="doubleCommentWrap" style="display: none;">
																					...
																					<c:choose>
																						<c:when test="${reReply.thumbStatus eq 'Y'}">
																							...
																							<img class="likeBtn3 shake"
																								src="${contextPath}/resources/img/like2.png" name="${reReply.replyNo}" id="${notice.boardNo}"
																								style="width: 25px !important; height: auto; margin-left: 2rem; bottom: 3px; left:10px;">
																						</c:when>
																						<c:otherwise>
																							<img class="likeBtn3 shake"
																								src="${contextPath}/resources/img/like.png" name="${reReply.replyNo}" id="${notice.boardNo}"
																								style="width: 25px !important; height: auto; margin-left: 2rem; bottom: 3px; left:10px;">
																						</c:otherwise>
																					</c:choose>
																				</div>
												...
												<c:if test="${memGradeInSt.citizenStatus eq 'Y'}">
													...
													<div class="inputCommentWrap" style="border-top: 1px solid #ced4da;">
														...
														<div style="width: 20%; display: inline-block; position: absolute;">
															<button type="submit" class="btn btn-secondary nanum reCommentBtn" id="${notice.boardNo}" name="${reply.replyNo}" 
